...
const shouldLoadApp = !href.includes('.json') && !href.includes('.xml')
...
function addToShopifyNav() {
  ...
  if (!nav) {
    navFound = false
    return alert(
      'Unable to find navigation to append the Poshify Utils button. Consider opening an issue at https://github.com/husseyexplores/poshify-utilities'
    )
  }
  ...
  extensionButton.querySelector('a > span').innerText = 'Poshify Utils'
...
window.addEventListener('load', () => {
  ...
  let addToNavInterval = setInterval(() => {
    if (!navFound) {
      clearInterval(addToNavInterval)
      addToNavInterval = null
      return
    }

    if (!isNavAdded) return addToShopifyNav()
    isNavAdded = true
  ...
  // If for some reason the interval doesnt clear itself
  setTimeout(() => {
    clearInterval(addToNavInterval)
    addToNavInterval = null
  }, 1000 * 20) // 20 sec
